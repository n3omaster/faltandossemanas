...
const BG_GIF_MOBILE =
  "https://media1.tenor.com/m/KTLkXNOTUooAAAAC/donald-trump-dance-holy-crap-donald-trump.gif";
const BG_GIF_DESKTOP =
  ...
  return (
    <main className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden font-['Space_Mono',monospace]">
      ...
      <div className="relative z-10 flex flex-col items-center justify-center flex-1 w-full px-4">
        <h1 className="text-white text-2xl sm:text-3xl md:text-4xl font-bold uppercase tracking-widest mb-4 sm:mb-6 text-center">
...
function TimeBlock({ value, label }: { value: number; label: string }) {
  return (
    <div className="flex flex-col items-center">
      <span className="text-white text-5xl sm:text-7xl md:text-9xl font-bold tabular-nums tracking-tight">
        {pad(value)}
      </span>
      <span className="text-blue-200 text-xs sm:text-sm md:text-base uppercase tracking-[0.3em] mt-2">
        {label}
      </span>
    </div>
  );
...
function Separator() {
  return (
    <span className="text-white/50 text-5xl sm:text-7xl md:text-9xl font-bold self-start">
      :
    </span>
  );
}
